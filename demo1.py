...
############# Define emoji , audio and text list ##############################
...
def draw_audio_text(frame,audio_text_to_show):
    '''
    画 audio 对应的文字
    '''
    cv2.putText(frame, audio_text_to_show, (10, 400), cv2.FONT_HERSHEY_PLAIN, 1.5, (211, 186, 85), 2)
...
class Parser(object):
    ...
    def draw_text(self, frame):
        ...
        if self.text_to_show is not None:
            cv2.putText(frame, self.text_to_show, (10, 50), cv2.FONT_HERSHEY_PLAIN, 1.5, (221, 221, 160), 2)
...
try:
    ...
    while True:
        ...
        if image is not None:
            ...
            frame[0:120, frame.shape[1]-160:frame.shape[1], :  ] = \
                frame[0:120, frame.shape[1]-160:frame.shape[1], :  ] * 0.6 + image * 0.4
        
        if samples is not None:
            label = predict_voice(samples)
            audio_text_to_show = random_choose(audio_text[label])
            draw_audio_text(frame,audio_text_to_show)
